...
class CIN(tf.keras.layers.Layer):
    ...
    def build(self, iput_shape):
        self.field_num = [input_shape[1]] + self.cin_size
        self.cin_W = [self.add_weight(name='w' + str(i),
                      shape=(1, self.field_num[0] * self.field_num[1], self.field_num[i + 1]),
                      initializer=tf.initializers.glorot_uniform(),
                      regularizer=tf.keras.regularizers.l1_l2(1e-5),
                      trainable=True) for i in range(len(self.field_num) - 1)]
    ...
    def call(self, inputs, **kwargs):
        ...
        for i, size in enumerate(self.field_num[1:]):
            ...
            x = tf.reshape(x, shape=[k, -1, self.field_num[0] * self.field_num[]])
...
class XDeepFM(tf.keras.Model):
    def __init__(self, feature_size, filed_size,
                 ...
                 cin_layers=[64, 32], l2_reg=0.001, zscore_file=None,
    ...
    def call(self, feat):
        ...
        if len(self.session_fea) > 0:
            ...
            for x in self.session_fea:
                ...
                cur_session_input = tf.reduce_sum(cur_session_input, 1)
        ...
        # cin part
        cin = self.cin_layers(dense)

        # concat lr & fm & deep
        concat = tf.concat([lr, fm, deep, cin], axis=1)
